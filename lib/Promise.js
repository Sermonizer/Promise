 ...
 * @LastEditTime: 2020-09-14 11:51:54
 * @LastEditors: Please set LastEditors
 * @Description: 自定义Promise模块 IIFE ES5
 ...
 */
  ...
  *   - resolve 成功的回调 成功的状态
  *   - reject 失败的回调 失败的状态
  *   - thorw error 抛出异常，也是失败的状态
  * 
  * 2. Promise指定多个then，都会调用吗？
  * let P = new Promise(...); p.then(...); p.then(...)
  *   - 会根据P的状态全部调用
  * 
  * 3. 改变Promise状态和指定回调函数谁先谁后？
  *   - 都有可能，一般是先指定回调，后改变状态
  *   - 可以通过直接resolve()、reject()来改变状态
  *   - 也可以延迟回调的调用(给then()加上定时器)
  * 
  * 4. 什么时候能得到数据呢?
  *   - 当先改状态，那么回调调用时就得到数据
  *   - 当先指定回调，当状态改变后回调被调用，得到数据
  * 
  * 5. Promise的执行是同步还是异步的？
  *   - resolve是同步执行的
  *   - .then()是同步执行的
  *   - .then里面的回调函数是异步执行的
  * 
  * 6. .then()返回的Promise状态由什么决定？
  *   - 由.then()指定的回调函数执行的状态决定
  *   - 如果抛出异常，那么返回的promise状态为rejected，reason为抛出的异常
  *   - 如果返回的是非promise的任意值，那么新promise的状态为resolved， value为返回值
  *   - 如果返回的是新promise，那么返回的结果由新promise的结果决定
  * 
  * 7. Promise如何实现异常传透？
  *   - 虽然.then()不用指定失败的回调函数，但是为了实现异常传透，.then()默认执行
  *     .then(value => {...}, reason => throw reason), 实现了异常穿透
  */
...
(function (window) {
   ...
   * race()：接收一个数组，返回一个Promise，其结果由第一个完成的Promise来决定
   */
